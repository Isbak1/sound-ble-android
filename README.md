...
**Sound and Bluetooth Recording** is implemented when the mobile application receives a record signal from the server.
...
**Sound and Bluetooth Synchronization** is implemented to reduce and calculate lag between the two separate recording from user's PC and mobile phone.
- Makes a call to our server’s dedicated time server.
- Receives browser’s recording start time, and compares it to the phone’s start time to calculate the lag and synchronize the sound and Bluetooth clips.
...
1. Register an account [here](https://sble.azurewebsites.net/login) and log in.
...
The repository for the web/server side of the Sound Bluetootth system can be found [here](https://github.com/Isbak1/sound-ble-web).
